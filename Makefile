...
IGCC_FLAGS      ?= -Wall -Wimplicit-function-declaration -Wformat=0 -flto -Isrc -Iinclude -larchive -fmodules -framework IOKit $(CFLAGS) 
ifdef RELEASE
...
IBTOOL_FLAGS    ?= --output-format human-readable-text  -Wimplicit-function-declaration --errors --warnings --notices --target-device iphone --target-device ipad $(IBFLAGS)
SIGN            ?= codesign
SIGN_FLAGS      ?= -s -
